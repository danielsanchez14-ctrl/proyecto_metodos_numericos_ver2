...
from core.methods.bisection_method import BisectionMethod
from utils.plotter import Plotter


class BisectionHandler(MethodHandler):
    ...
    def execute(self, raw_values: dict[str, str]) -> MethodResult:
        ...
        table, root, iterations = BisectionMethod.compute(f, xl, xu, tol)
        df = pd.DataFrame(table)
        # 4. Generar gráfica de la función
        x_vals = np.linspace(xl, xu, 1000)
        y_vals = [f(x) for x in x_vals]
        function_fig = Plotter.plot_function(x_vals, y_vals, "Gráfica de la función", "x", "f(x)", texto_f)

        # 5. Generar gráfica del error
        error_col = "Error: |Xu - Xr|"
        error_fig = Plotter.plot_function(
            list(range(iterations)),
            table[error_col],
            f"Evolución del {error_col}",
            "Nún. de iteración",
            "Error",
            error_col
        )
